...
def rgb_to_hls(rgb_array: numpy.ndarray) -> numpy.ndarray:
    ...
    h = (
        numpy.where(
            ...
            numpy.where(
                r == maxc, bc - gc,
                numpy.where(g == maxc, 2.0 + rc - bc, 4.0 + gc - rc)
            )
        ) / 6
    ) % 1
    l = sumc / 2.0
    with numpy.errstate(divide = 'ignore', invalid = 'ignore'):
        s = numpy.where(
            minc == maxc, 0, numpy.where(l < 0.5, rangec / sumc, rangec / (2.0 - sumc))
        )
...
def hls_to_rgb(hls_array: numpy.ndarray) -> numpy.ndarray:
    ...
    def _v(m1, m2, h):
        h = h % 1.0
        return numpy.where(
            h < ONE_SIXTH, m1 + (m2 - m1) * h * 6,
            numpy.where(
                h < .5, m2,
                numpy.where(h < TWO_THIRD, m1 + (m2 - m1) * (TWO_THIRD - h) * 6, m1)
            )
        )
...
def hsv_to_rgb(hsv_array: numpy.ndarray) -> numpy.ndarray:
    ...
    return wh(
        ...
        wh(
            i == 1, numpy.concatenate((q, v, p), axis = 1),
            wh(
                i == 2, numpy.concatenate((p, v, t), axis = 1),
                wh(
                    i == 3, numpy.concatenate((p, q, v), axis = 1),
                    wh(
                        i == 4, numpy.concatenate((t, p, v), axis = 1),
                        wh(
                            i == 5, numpy.concatenate((v, p, q), axis = 1),
