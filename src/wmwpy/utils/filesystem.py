...
class Filesystem():
    ...
    def getAssets(
        ...
        for dir, subdir, files in os.walk(assets):
            for file in files:
                path = pathlib.Path(
                    '/', os.path.relpath(os.path.join(dir, file), assets)
                ).as_posix()
                # print(path)
...
class File(FileBase):
    ...
    def setReader(self, extension = None, mime = None, **kwargs):
        ...
        for r in FILE_READERS:
            # print(r)
            if r.check(
                mime, extension, self.rawdata, filesystem = self.filesystem, **kwargs
            ):
                self.reader = r
                return r
...
class Folder(FileBase):
    ...
    def add(
        ...
        if len(parts) > 1:
            ...
            return file.add(
                pathlib.Path(*parts[1::]).as_posix(), content, replace = replace
            )
        else:
            if file != None:
                if not replace:
