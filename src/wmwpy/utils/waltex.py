...
class _WaltexImageFile(ImageFile.ImageFile):
    ...
    def _open(self):
        ...
        if fmt == 0:
            ...
            self.tile = [
                ("raw", (0, 0) + self.size, self.HEADER_LENGTH, (self.mode, 0, 1))
            ]
        elif fmt == 3:
            # RGBA4444
            # Use raw decoder with custom RGBA;4B unpacker
            self.tile = [
                ("raw", (0, 0) + self.size, self.HEADER_LENGTH, ('RGBA;4B', 0, 1))
            ]
        elif fmt in [1, 2]:
...
def WaltexImage(
    path: str | bytes | io.BytesIO,
    premultiplyAlpha: bool = False,
    dePremultiplyAlpha: bool = False,
    endian: str = 'little'
) -> Image.Image:
    """
    ### Depracted
