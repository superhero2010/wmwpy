...
try:
    ...
    class _WaltexFile(filetype.Type):
        ...
        def __init__(self):
            ...
            """
            super(_WaltexFile, self).__init__(mime=_WaltexFile.MIME, extension=_WaltexFile.EXTENSION)

        def match(self, buf):
            return (len(buf) > 3 and buf[0] == 0x57 and buf[1] == 0x41 and buf[2] == 0x4C and buf[3] == 0x54)
...
Image.register_extensions(_WaltexImageFile.format, [".waltex"])
...
class Waltex():
    ...
    def __init__(this, file : str | bytes, byte_order : str = 'little') -> None:
        ...
        this._colorspecs = [
            {'order': 'rgba', 'bpp': [8, 8, 8, 8], 'spec': 'rgba8888', 'byte_order': 'big'},
            {'order': 'rgb', 'bpp': [5, 6, 5], 'spec': 'rgb565', 'byte_order': 'little'},
            {'order': 'rgba', 'bpp': [5, 5, 5, 1], 'spec': 'rgba5551', 'byte_order': 'little'},
            {'order': 'rgba', 'bpp': [4, 4, 4, 4], 'spec': 'rgba4444', 'byte_order': 'little'}
        ]
